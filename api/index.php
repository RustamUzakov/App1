<?php
	...
	switch( $dataSource ){
		...
		case "userProfile":
			...
			break;
		case "pullHourlyRecord":
			$result['status'] = $userMgr->pullHourlyRecord( $request['totalM'],$request['totalT'],$request["totalW"],$request['totalTh'],$request['totalF'],$request['totalS'],$request['totalSun'],$request['totalWeek'] );
			break;
		case "deleteRecord":
			$result['status']= $userMgr->deleteRecord( $request['id']);
				break;
	}	
